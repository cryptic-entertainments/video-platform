...
if (isset($user_address)) {
    $result2 = mysqli_query($con, "SELECT * FROM `favourite_videos` INNER JOIN `video_info` ON `favourite_videos`.`video_info_id`=`video_info`.`video_uuid` WHERE `user_id`= '$user_address' ORDER BY `favourite_videos`.`favourite_video_id` DESC");
...
<head>
    ...
    <title>Favourite Videos - Cryptic Entertainments</title>
    <!-- Favicon -->
