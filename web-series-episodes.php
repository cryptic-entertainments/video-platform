...
$video_uuid = '';
$total_count = 0;
if (isset($_GET['video_uuid'])) {
    $video_uuid = $_GET['video_uuid'];
    $result2 = mysqli_query($con, "SELECT * FROM `web-series-episodes-info` WHERE `web_series_uuid` = '$video_uuid'");
...
<body>
    ...
    ?>
                    ...
                    <div class="col-lg-12">
                        <nav aria-label="breadcrumb">
                            <div class="gen-breadcrumb-title">
                                <?php
                                $query = "SELECT name_of_web_series FROM `web_series_info` WHERE web_series_uuid = '$video_uuid'";
                                $result = mysqli_query($con, $query);
                                if (mysqli_num_rows($result) > 0) {
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $current_module_name = $row['name_of_web_series'];
                                    }
                                }
                                ?>
                                <h1>
        ...
        <input type="hidden" id="video_uuid" value="<?= $video_uuid ?>">
    ...
    <script>
        ...
        function handleClick(id) {
            $.ajax({
                type: 'POST',
                url: 'php/superPass.php',
                data: {
                    "super_pass": "0x8b0e1b0c0e1b0c0e1b0c0e1b0c0e1b0c0e1"
                },
                success: function(data) {
                    console.log("Success");
                }
